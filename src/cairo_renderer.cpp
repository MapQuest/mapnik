 ...
 */
struct cairo_point_render_thunk : public common_point_render_thunk
{
    composite_mode_e comp_op_;

    cairo_point_render_thunk(pixel_position const &pos, marker const &m,
                             agg::trans_affine const &tr, double opacity,
...
struct cairo_text_render_thunk : public common_text_render_thunk
{
    composite_mode_e comp_op_;
    
    cairo_text_render_thunk(placements_list const &placements,
                            composite_mode_e comp_op)
        : common_text_render_thunk(placements), comp_op_(comp_op)
    {}
};

// Variant type for render thunks to allow us to re-render them
// via a static visitor later.
typedef boost::variant<cairo_point_render_thunk,
                       cairo_text_render_thunk> render_thunk;
 ...
 */
struct cairo_render_thunk_extractor : public boost::static_visitor<>,
                                    public common_render_thunk_extractor<render_thunk>
{
	cairo_render_thunk_extractor(box2d<double> &box,
	...
	void operator()(point_symbolizer const &sym) const
    {
        ...
        render_point_symbolizer(
            ...
            [&](pixel_position const &pos, marker const &marker,
                agg::trans_affine const &tr, double opacity) {
                cairo_point_render_thunk thunk(pos, marker, tr, opacity, comp_op);
    ...
    void operator()(text_symbolizer const &sym) const
    {
        ...
        placements_list const& placements = helper.get();
        cairo_text_render_thunk thunk(placements, comp_op);
...
struct thunk_renderer : public boost::static_visitor<>
{
    ...
    void operator()(cairo_point_render_thunk const &thunk) const
    {
    ...
    void operator()(cairo_text_render_thunk const &thunk) const
    {
        cairo_save_restore guard(context_);
                                  ...
                                  proj_transform const& prj_trans)
{
    ...
    render_group_symbolizer<decltype(render_thunks), cairo_render_thunk_extractor>(
        sym, feature, prj_trans, common_.query_extent_, common_, render_thunks);
}
